...
sql += "insert into public.products (name, short_description, category, image_url, is_active) values \n";

const values = products.map((p) => {
    let img = p.image_url ? `'${p.image_url}'` : 'null';
    let name = p.name.replace(/'/g, "''");
    let desc = p.short_description.replace(/'/g, "''");
    let cat = p.category.replace(/'/g, "''");

    return `('${name}', '${desc}', '${cat}', ${img}, ${p.is_active})`;
});
...
console.log("SQL successfully written to generated_seed.sql");
